...
def handle_events():
    ...
    for event in events:
        ...
        elif event.type == SDL_MOUSEBUTTONDOWN and event.button == SDL_BUTTON_LEFT: # 좌클릭하면
            points.append((event.x, TUK_HEIGHT - 1 - event.y)) # 클릭된 위치를 새 점으로 추가
        elif event.type == SDL_KEYDOWN and event.key == SDLK_ESCAPE:
            running = False
    pass

def reset_world():
    global running, cx, cy, frame, t, action, mx, my, points
    mx, my = 0, 0
    running = True
...
def set_new_target_arrow():
    ...
    if points: # points 리스트 안에 남아있는 점이 있으면 :
        ...
        action = 3 if action == 1 else 2 # 이전에 소년이 이동중이었던 방향 따라 idle.
        frame = 0
        target_exists = False
...
def update_world():
    ...
    if target_exists:
        if t <= 1.0:
            ...
            set_new_target_arrow()
    elif points: # 목표지점이 없을때 새 목표지점이 생기면...
        set_new_target_arrow()
